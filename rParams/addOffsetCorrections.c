...
*/
         if(inputImage->lookDir == LEFT) dlat = -0.02; else dlat = 0.02; 
...
*/
         da = (azimuth2-azimuth1)*inputImage.nAzimuthLooks * 
              AzimuthPixelSize;
         if(inputImage.passType == DESCENDING) da *= -1.0;
/*
...
*/
         if(inputImage.lookDir == RIGHT) hAngle = atan2(da,dgr);
         else if(inputImage.lookDir==LEFT) hAngle = atan2(da,-dgr);
         else error("*** computeHeading: invalid lookDir ***");
/*
   Compute angle for ps coordinats from north
*/
         rot = 45.0;
         if(lat < 0) rot=0.0; /* Use rot=0 for southern latitudes */
