...
void rParamsCoeffs(void *x,int i,double *afunc, int ma);
void fewPoints();
void rParamsCoeffsQuad(void *x,int i,double *afunc, int ma);

void rbnbpdBpParamsCoeffs(void *x,int i,double *afunc, int ma);
    
void rbpdBpParamsCoeffs(void *x,int i,double *afunc, int ma); 
void constOnlyParamsCoeffs(void *x,int i,double *afunc, int ma);
void BpCorrectOnlyParamsCoeffs(void *x,int i,double *afunc, int ma);
void rParamsCoeffsQuadCorrect(void *x,int i,double *afunc, int ma);
...
{   
	...
	modelValues *x;
	int azimuth;
	int i,i1,k,j,npts,l1,l2, kPrint;
	...
	*/
	...
	for(k=0; k<=kPrint; k++) {
		if(k == 0) {
			if(tiePoints->deltaB == DELTABNONE) {
				...
				Bn=0.0; Bp =0.0; dBn= 0.0;  dBp =0.0;	dBnQ=0.0; dBpQ=0.0;
				bnFix=0.0; bpFix =0.0;
			}
		} 
		varP=0.0; meanP=0.0;
		...
		if(tiePoints->deltaB == DELTABNONE) {
			if(tiePoints->bnbpdBpFlag == TRUE) { 		/*    solve for bn,bp,dBp */
				if(npts < 3) fewPoints();
				svdfit((void *)x,y,sig,npts,a,ma,u,v,w, &chisq,&rbnbpdBpParamsCoeffs);
				Bn=a[1]; Bp=tiePoints->BpCorig; dBn=tiePoints->dBnorig; dBp=a[2]; dBnQ=0.,dBpQ=0.; cnst=a[3];
				pIndex[1]=1;  pIndex[2]=0; pIndex[3]=2; pIndex[4]=0; pIndex[5]=0; pIndex[6]=3;

			} else  if(tiePoints->bpdBpFlag == TRUE) { 			/*	  Solve for bp and dBp only 			*/
				if(npts < 2) fewPoints();
				svdfit((void *)x,y,sig,npts,a,ma,u,v,w, &chisq,&rbpdBpParamsCoeffs);
				Bn=tiePoints->BnCorig; Bp=tiePoints->BpCorig; dBn=tiePoints->dBnorig; dBp=a[1]; dBnQ=0.,dBpQ=0.; cnst=a[2];
				pIndex[1]=0;  pIndex[2]=0; pIndex[3]=1; pIndex[4]=0; pIndex[5]=0; pIndex[6]=2;
			} else if(tiePoints->constOnlyFlag == TRUE) {
				if(npts < 2) fewPoints();
				svdfit((void *)x,y,sig,npts,a,ma,u,v,w, &chisq,&constOnlyParamsCoeffs);
				Bn=tiePoints->BnCorig; Bp=tiePoints->BpCorig; dBn=tiePoints->dBnorig; dBp=tiePoints->dBporig; dBnQ=0.,dBpQ=0.; cnst=a[1];
				...
				Bn=a[1]; Bp=tiePoints->BpCorig; dBn=a[3]; dBp=a[2]; dBnQ=a[6],dBpQ=a[5]; cnst=a[4];
				pIndex[1]=1;  pIndex[2]=3; pIndex[3]=2; pIndex[4]=6; pIndex[5]=5; pIndex[6]=4;
			}  else { 			/* 	 SOLVE FOR ALL PARAMETERS */
				if(npts < 4) fewPoints();
				svdfit((void *)x,y,sig,npts,a,ma,u,v,w, &chisq,&rParamsCoeffs);
				Bn=a[1]; Bp=tiePoints->BpCorig; dBn=a[3]; dBp=a[2]; dBnQ=0.0,dBpQ=0.0; cnst=a[4];
				pIndex[1]=1;  pIndex[2]=3; pIndex[3]=2; pIndex[4]=0; pIndex[5]=0; pIndex[6]=4;
			...
			cnst=0.0;
			Bn=a[1]; Bp=a[4]; dBn=a[3]; dBp=a[2]; dBnQ=a[6],dBpQ=a[5]; 
			pIndex[1]=1;  pIndex[2]=3; pIndex[3]=2; pIndex[4]=6; pIndex[5]=5; pIndex[6]=4;
		} else error("computeRParams: invalid deltaB flag ",tiePoints->deltaB);
		svdvar(v,ma,w,Cp);
		fprintf(stderr," --- %f %f %f %f %f %f %f\n",Bn,Bp,dBn,dBp,dBnQ,dBpQ,cnst);
	} 
	...
	if(tiePoints->deltaB == DELTABNONE ) {
		...
		if(tiePoints->bnbpdBpFlag == TRUE) {
			...
			fprintf(stdout,"%11.5f  %11.5f  %11.5f  %f %f %f %f\n&\n",tiePoints->BnCorig,tiePoints->BpCorig,tiePoints->dBnorig,a[1],0.0,0.0,a[2]);
		} else if(tiePoints->constOnlyFlag == TRUE) {
			fprintf(stdout,"%11.5f  %11.5f  %11.5f  %f %f %f %f\n&\n",tiePoints->BnCorig,tiePoints->BpCorig,tiePoints->dBnorig, tiePoints->dBporig,a[1],0.0,0.0);
		} else if(tiePoints->quadB == TRUE) {
