...
{   
	...
	*/
	...
	if(outputImage.rOffsetFlag==TRUE)  fprintf(stderr,"*** USING RANGE/AZIMUTH OFFSETS in solution *** \n");
	...
	*/
	...
	if(landSatFile != NULL && LSImages !=NULL) {
		if(statsFlag==TRUE) error("Landsat  incompatible with stats flag, which is for speckle tracked offsets only\n");
	...
	for (i=0; i < outputImage->ySize; i++) {
		...
		for(j=0; j < outputImage->xSize; j++) {
			...
			if(refVel->initMapFlag==TRUE ) {
				...
				if(outputImage->timeOverlapFlag==TRUE) error("can't use reference map and timeoverlap flag at the same time");
				refVelInterp(x,y, refVel,&vxPt, &vyPt,&exPt,&eyPt);
				if(vxPt > -2e6 && vxPt < 2e6) {
					scaleX[i][j]=1.0/(exPt*exPt); vXimage[i][j]=vxPt;
					scaleY[i][j]=1.0/(eyPt*eyPt); vYimage[i][j]=vyPt;
					scaleZ[i][j]=0.0; vZimage[i][j]=0.0;
					errorX[i][j]=exPt*exPt; errorY[i][j]=eyPt*eyPt; 
				}
			} else {
...
static void get3DProj(	inputImageStructure *ascImages, inputImageStructure *descImages,int nAsc, int nDesc,int northFlag, outputImageStructure *outputImage  ) {
	...
	if(nAsc != 0) lat = ascImages->latControlPoints[1];
	else if(nDesc !=0) lat=descImages->latControlPoints[1];
	else {
		if(northFlag == FALSE) {
...
static void logInputFiles3d(outputImageStructure *outputImage,char **geodatFiles, char **phaseFiles, char **baselineFiles,  char **rOffsetFiles, char **rParamsFiles, char **offsetFiles, char **azParamsFiles,float *nDays, float *weights, int *crossFlags, int nFiles, int offsetFlag){
	...
	for(i=0; i < nFiles; i++) {
		if(geodatFiles[i] != NULL) {
			...
			else fprintf(outputImage->fpLog,"; Range Offsets : %s\n; Rbaseline     : %s\n",     "none","none");
		}
	}
	fflush(outputImage->fpLog);
}
...
static void logInputs3d(outputImageStructure *outputImage, char *outFileBase,char *inputFile, char *demFile, char *irregFile, char *shelfMaskFile, char *extraTieFile, char *tideFile,float fl,    int statsFlag, int makeTies,
	...
	if(outputImage->fpLog==NULL) error("Could not open log file %s",logFile);
	...
	else fprintf(outputImage->fpLog,"; irregFile        : %s\n","None");
	if(shelfMaskFile != NULL) 
...
static void write3Doutput( outputImageStructure outputImage, char *outFileBase )
{
	...
	/* Output file names velocity */	
	outFileEx = (char *) malloc(LINEMAX);   outFileEx[0] ='\0'; 
	outFileEx = strcat(outFileEx,outFileBase);   
	outFileEx = strcat(outFileEx,".ex");
	outFileEy = (char *) malloc(LINEMAX);   outFileEy[0] ='\0'; 
	...
	*/	
	if( outputImage.timeOverlapFlag==TRUE  )  {
		...
		/* recast pointers */
		vx=(float **)outputImage.image; vy=(float **)outputImage.image2;  dT=(float **)outputImage.image3;				
		for (i=0; i < outputImage.ySize; i++) {
			for(j=0; j < outputImage.xSize; j++) {
				/* Ensure only data in date range written */
				if(dT[i][j] > maxdT || dT[i][j] < (-maxdT)) {
					vx[i][j]=-2.e9;  vy[i][j]=-2.e9;   dT[i][j]=-2.e9;
	...
	free(outputImage.image[0]); free(outputImage.image);                  
...
static void processMosaicDate(outputImageStructure *outputImage, char *date1,char *date2)
{
	...
	if(date1 != NULL ) {
		if(sscanf(date1,"%2d-%2d-%4d",&m1,&d1,&y1) != 3) error("invalid date %s\n",date1);
		outputImage->jd1= juldayDouble(m1,d1,y1);
		fprintf(stderr,"%i %i %i %lf\n",m1,d1,y1,outputImage->jd1);
	} else outputImage->jd1=0.0;

	if(date2 != NULL ) {
		if(sscanf(date2,"%2d-%2d-%4d",&m2,&d2,&y2) != 3) error("invalid date %s\n",date2);
...
{     
	...
	*/
	metaFile = (char *) malloc(LINEMAX);  metaFile[0] ='\0'; 
		...
		if(imageTmp->used==TRUE) { /* Only output images that were used */
			...
			*/
			...
			/*
			  Nominal time
			*/
			dday=(imageTmp->julDay-(long)imageTmp->julDay);
			hour=(int)(dday*24);
			minute=(int)((dday*24-hour)*60);
			second=(int)(dday*86400-hour*3600-minute*60 + .5);
			tTmp.tm_sec=second; tTmp.tm_hour=hour; tTmp.tm_min=minute;
			...
			*/
			if(strstr(tmp,"TDX") != NULL) strcat(sensor,"TSX/TDX");
			else if(strstr(imageTmp->par.label,"TSX") !=NULL) strcat(sensor,"TSX/TDX");
			else strcat(sensor,"not_specfied");
	...
	*/
	...
	if(haveData==FALSE) remove(metaFile);
	return(haveData);
}
		     ...
		     char **date1,char **date2, referenceVelocity *refVel, int *statsFlag, outputImageStructure *outputImage)
{
	...
	*/
	...
	if(refVel->initMapFlag == TRUE && refVel->velFile == NULL) error("initMap set but no reference velocity provided\n");
	if( *statsFlag==TRUE) {
...
{ 
	error("\033[1m\n\n%s\n\n%s\n\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n\n\n",
	      ...
	      "\tSVConst =\t\t\t If available, use const only state vector solution",	      	      
...
static void mallocOutputImage(outputImageStructure *outputImage)
{
	...
	buf1=(float *)malloc(bufSize );  buf2=(float *)malloc(bufSize );
	buf3=(float *)malloc(bufSize );  buf1s=(float *)malloc(bufSize);
	buf2s=(float *)malloc(bufSize);  buf3s=(float *)malloc(bufSize);
	bufx=(float *)malloc(bufSize);    bufy=(float *)malloc(bufSize);
	bufz=(float *)malloc(bufSize);    bufs=(float *)malloc(bufSize);
	bufsx=(float *)malloc(bufSize);   bufsy=(float *)malloc(bufSize);
	bufex=(float *)malloc(bufSize);   bufey=(float *)malloc(bufSize);
...
static void readReferenceVelMosaic(referenceVelocity *refVel, outputImageStructure *outputImage)
{
	...
	*/
	...
	geodatFile = (char *)malloc(strlen(refVel->velFile)+11); 	geodatFile[0]='\0';
	...
	/* 
	   velocity file names
	*/
	vxFile = (char *)malloc(strlen(refVel->velFile)+4); 	vxFile[0]='\0';
	vxFile=strcpy(vxFile,refVel->velFile);	vxFile=strcat(vxFile,".vx");
	vyFile = (char *)malloc(strlen(refVel->velFile)+4); 	vyFile[0]='\0';
	vyFile=strcpy(vyFile,refVel->velFile);	vyFile=strcat(vyFile,".vy");
	fprintf(stderr,"vx,vy file %s %s\n",vxFile,vyFile);
	/*
	  Open geodat file
	*/
	fp = openInputFile(geodatFile);
	if(fp == NULL) error("*** readREfVelFile: Error opening %s ***\n",geodatFile);
	/*
	  Read parameters
	*/
	...
	*/
	...
	/*
	  Open vy file
	*/
	if(vyFile == NULL)  error("*** readVel: Error opening %s ***\n",vyFile);
	fp = openInputFile(vyFile);
	fseek(fp, (nx*yoff*sizeof(float)),SEEK_SET);
	if(fp == NULL) error("*** readVel: Error opening %s ***\n",vyFile);
	for(i=0; i < refVel->ny; i++) {
		tmp1 =&(tmp[i*refVel->nx]);
		fseek(fp,(xoff*sizeof(float)),SEEK_CUR);
		freadBS(tmp1,sizeof(float),refVel->nx,fp,FLOAT32FLAG);
	...
	if(refVel->initMapFlag == TRUE ) {
		/* 
		   velocity file names
		*/
		exFile = (char *)malloc(strlen(refVel->velFile)+4); 	exFile[0]='\0';
		exFile=strcpy(exFile,refVel->velFile);	exFile=strcat(exFile,".ex");
		eyFile = (char *)malloc(strlen(refVel->velFile)+4); 	eyFile[0]='\0';
		eyFile=strcpy(eyFile,refVel->velFile);	eyFile=strcat(eyFile,".ey");
		/*
		  Malloc array
		*/
		...
		/*
		  Open ex file
		*/
		if(exFile == NULL)  error("*** readVelMosaic: Error opening %s ***\n",exFile);
		...
		*/
		...
		/*
		  Open ey file
		*/
		if(eyFile == NULL)  error("*** readVelMosaic: Error opening %s ***\n",eyFile);
