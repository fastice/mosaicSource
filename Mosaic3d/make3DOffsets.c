...
{   
	...
	*/
	...
	nTotal=0;
	for(aOffImage=allImages; aOffImage != NULL; aOffImage=aOffImage->next) { nTotal++;}
	...
	for(aOffImage=allImages; aOffImage != NULL; aOffImage=aOffImage->next, aParams=aParams->next) {
		...
		*/
		...
		for(dOffImage=aOffImage->next; dOffImage != NULL; dOffImage=dOffImage->next, dParams=dParams->next) {
			...
			if( (dOffImage->passType == aOffImage->passType && sepAscDesc == TRUE) || dOffImage->crossFlag==FALSE) continue;
			/*
				Check images close enough in time
			*/
			if(fabs(aOffImage->julDay - dOffImage->julDay) > timeThresh || dOffImage->weight < 0.05 || dParams->offsets.rFile == NULL) continue;    
