...
*/
void writeTieFile(outputImageStructure *outputImage, xyDEM *dem, xyDEM *vzCorrect,
				  char *outFile, double tieThresh, char *extraTieFile, char *tideFile, int32_t autoSize)
{
	...
	for (i = 0; i < outputImage->ySize; i++)
	{
		...
		for (j = 0; j < outputImage->xSize; j++)
		{
			...
			if (v < tieThresh && errorV < eThresh && errorV > 0.0 && fabs(vZimage[i][j]) < 0.04 * tieThresh)
			{
				...
				{
					vz = vZimage[i][j] + interpVCorrect(x, y, vzCorrect);
				}
				/* fprintf(stderr,"%f\n", vz); */
				if (sMask == SHELF)
					vz += tideCorrection;
				fprintf(fp, " %10.5f %10.5f  %7.1f    %8.2f %8.2f %8.3lf\n", lat, lon, z,
						vXimage[i][j], vYimage[i][j], vz);
			}
		}
	}
	/*
...
static void writeExtraTies(char *extraTieFile, ShelfMask *shelfMask, double tideCorrection, xyDEM *dem, FILE *fp, double xmin, double ymin, double xmax, double ymax)
{
	...
	while (line != NULL && strchr(line, '&') == NULL)
	{
		line = fgets(buf, 512, fpIn);
		weight = 1.0;
		n = sscanf(line, "%lf%lf%lf%lf%lf%lf%lf\n", &lat, &lon, &z, &vx, &vy, &vz, &weight);
		if (n != 7)
		{
		...
		else
		{
			...
			if (x >= xmin && x <= xmax && y >= ymin && y <= ymax)
				fprintf(fp, "%10.5f %10.5f  %7.1f    %8.2f %8.2f %8.3f %9.2f\n", lat, lon, z, vx, vy, vz, weight);
		}
	}
	fclose(fpIn);
}
