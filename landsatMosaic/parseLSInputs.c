	...
	*/
	...
	while( notdone == TRUE ) {                 /* Loop to read lines */
		...
		if( strchr(line,ENDDATA) != NULL || lineLength ==0) {
			...
			*/
			tmpImage=(landSatImage *)malloc(sizeof(landSatImage)); tmpImage->next=NULL;
			if(LSImages==NULL) { firstImage=tmpImage; prevImage=NULL; LSImages=tmpImage; firstImage->nImages=0;} 
			else {LSImages->next=tmpImage; prevImage=LSImages;  LSImages=tmpImage;}
			/*
			  Parse mask file
			*/
			if ( maskFile != NULL ) {
				LSImages->maskFile=(char *)malloc(sizeof(char)*(strlen(maskFile)+1)); LSImages->maskFile[0]='\0';
			...
			/*
			  Parse match data
			*/
			LSImages->fitResult.matchFile=(char *)malloc(sizeof(char)*(strlen(matchFile)+1)); LSImages->fitResult.matchFile[0]='\0';
			...
			*/
			...
			if( (timeOverlapFlag == TRUE && tmpWeight > 0.05) || (timeOverlapFlag==FALSE && tmpWeight > 0.99) ) {
				...
				*/
				if (fitFile == NULL) {
					LSImages->fitResult.fitFile=(char *)malloc(sizeof(char)*(strlen(line)+1+4)); LSImages->fitResult.fitFile[0]='\0';
					strcpy(LSImages->fitResult.fitFile,matchFile); 
					strcat(LSImages->fitResult.fitFile,".fit");
				} else {
					LSImages->fitResult.fitFile=(char *)malloc(sizeof(char)*(strlen(fitFile)+1)); LSImages->fitResult.fitFile[0]='\0';				
	...
	fclose(fp);
	if(firstImage != NULL) fprintf(stderr,"Exit ParseLSInputs %i\n", firstImage->nImages); else fprintf(stderr,"Exit ParseLSInputs NULL\n");
	free(line);
	return(firstImage);
...
static void parseFitFile(lsFit *fitDat)
{
	...
	*/
	...
	for(i=0; i < 3; i++){
        ...
        for(i=0; i < MAXFITPARAM; i++) { fitDat->pX[i]=NODATA; fitDat->pY[i]=NODATA; }
	while( notdone == TRUE && lineCount < 30) {                 /* Loop to read lines */
		...
		if( strchr(line,ENDDATA) != NULL ) 
			...
			if( strchr(line,'=') != NULL ) {
				...
				if(strstr(keyword,"Xfit") != NULL) {
					if(sscanf(value,"%lf%lf%lf\n",&fdum1,&fdum2,&fdum3) != 3) error("parse fit X fit");
					fitDat->pX[0]=fdum1;	fitDat->pX[1]=fdum2; 	fitDat->pX[2]=fdum3;
				}
				if(strstr(keyword,"Yfit") != NULL) { 
					if(sscanf(value,"%lf%lf%lf\n",&fdum1,&fdum2,&fdum3) != 3) error("parse fit Y fit"); 
					fitDat->pY[0]=fdum1;	fitDat->pY[1]=fdum2; 	fitDat->pY[2]=fdum3;
				}
				/*
				  Read residuals in units of meters
				*/
				if(strstr(keyword,"X_residual") != NULL) { 
					if(sscanf(value,"%lf%lf%lf\n",&fdum1,&fdum2,&fdum3) != 3) {fitDat->sigmaXRes=NODATA;  }
					else fitDat->sigmaXRes=fdum3;
				}
				if(strstr(keyword,"Y_residual") != NULL) { 
					if(sscanf(value,"%lf%lf%lf\n",&fdum1,&fdum2,&fdum3) != 3)  {fitDat->sigmaYRes=NODATA;  }
					else fitDat->sigmaYRes=fdum3;
				}

				if(strstr(keyword,"CX_11_12_13") != NULL) { 
					if(sscanf(value,"%lf%lf%lf\n",&fdum1,&fdum2,&fdum3) == 3)  {
						fitDat->Cx[0][0]=fdum1; fitDat->Cx[0][1]=fdum2;   fitDat->Cx[0][2]=fdum3;
					}
				}

				if(strstr(keyword,"CX_21_22_23") != NULL) { 
					if(sscanf(value,"%lf%lf%lf\n",&fdum1,&fdum2,&fdum3) == 3)  {
						fitDat->Cx[1][0]=fdum1; fitDat->Cx[1][1]=fdum2;   fitDat->Cx[1][2]=fdum3;
					}
				}

				if(strstr(keyword,"CX_31_32_33") != NULL) { 
					if(sscanf(value,"%lf%lf%lf\n",&fdum1,&fdum2,&fdum3) == 3)  {
						fitDat->Cx[2][0]=fdum1; fitDat->Cx[2][1]=fdum2;   fitDat->Cx[2][2]=fdum3;
					}
				}
				if(strstr(keyword,"CY_11_12_13") != NULL) { 
					if(sscanf(value,"%lf%lf%lf\n",&fdum1,&fdum2,&fdum3) == 3)  {
						fitDat->Cy[0][0]=fdum1; fitDat->Cy[0][1]=fdum2;   fitDat->Cy[0][2]=fdum3;
					}
				}

				if(strstr(keyword,"CY_21_22_23") != NULL) { 
					if(sscanf(value,"%lf%lf%lf\n",&fdum1,&fdum2,&fdum3) == 3)  {
						fitDat->Cy[1][0]=fdum1; fitDat->Cy[1][1]=fdum2;   fitDat->Cy[1][2]=fdum3;
					}
				}

				if(strstr(keyword,"CY_31_32_33") != NULL) { 
					if(sscanf(value,"%lf%lf%lf\n",&fdum1,&fdum2,&fdum3) == 3)  {
						fitDat->Cy[2][0]=fdum1; fitDat->Cy[2][1]=fdum2;   fitDat->Cy[2][2]=fdum3;
					}
				}
			} /* end if line=... */
		} /* E nd else */
	} /* End while */
	if(lineCount>=30) error("> 30 lines in fit file, likely error %s \n", fitDat->fitFile);

	for(i=0; i < 3; i++){
		for(j=0; j < 3; j++){
			/*fprintf(stderr,"%f %f \n",fitDat->Cx[i][j],fitDat->Cy[i][j]);*/
			if(fitDat->Cx[i][j]==LARGEINT ||  fitDat->Cy[i][j]==LARGEINT) error("-- Missing Covarance Elements--");
		}
	}

	for(i=0; i < MAXFITPARAM; i++) { 
		if(fitDat->pX[i]==NODATA ||  fitDat->pY[i]==NODATA) error("Error reading fit");
	}
