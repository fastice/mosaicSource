...
void readLSOffsets(lsFit *fitDat, matchResult *matches, int32 readData, char *maskFile)
{
	...
	*/
	offsetDatFile = (char *)malloc(strlen(fitDat->matchFile)+5); 	offsetDatFile[0]='\0';
	...
	/*
	  Read data X data
	*/
	if(readData == FALSE) return;  /* This option is so file read up front in mosaicker doesn't leak memory */
	fprintf(stderr,"Read -  X offsets \n");
	offXFile = (char *)malloc(strlen(fitDat->matchFile)+4); 	offXFile[0]='\0';
	...
	*/
	...
	offYFile = (char *)malloc(strlen(fitDat->matchFile)+4); 	offYFile[0]='\0';
	...
	*/
	fprintf(stderr,"Read - Y offsets \n");
	typeFile = (char *)malloc(strlen(fitDat->matchFile)+7); 	typeFile[0]='\0';
...
float **LSreadFloatImage(char *file, int32 nx, int32 ny)
{
	...
	*/
	...
	/* 
	   read file
	*/
	for(i=0; i< ny; i++) { freadBS(image[i],sizeof(float),(size_t)(nx),fp, FLOAT32FLAG); 	 }
	fclose(fp);
	return(image);
}
...
static void parseLSOffsetMeta(char *datFile,  lsFit *fitDat,matchResult *matches )
{
	...
	*/
	notdone=TRUE;
	matches->x0=NODATA;	matches->y0=NODATA;
	matches->dx=NODATA;	matches->dy=NODATA;
	matches->nx=0;	matches->ny=0;
	fitDat->proj=-1;
		...
		if( strchr(line,ENDDATA) != NULL ) 
			...
			if( strchr(line,'=') != NULL ) {
				...
				if(strstr(keyword,"fileEarly") != NULL ) { 
					for(i=0; i<strlen(value); i++) if(value[i]=='\n' || value[i]=='\r') value[i]='\0'; /* strip carriage return */
				...
				if(strstr(keyword,"x0") != NULL) { if(sscanf(value,"%lf\n",&fdum) != 1) error("parse LS Offsets x0");  matches->x0=fdum; }
				if(strstr(keyword,"y0") != NULL) { if(sscanf(value,"%lf\n",&fdum) != 1) error("parse LS Offsets y0"); matches->y0=fdum; }
				if(strstr(keyword,"dx") != NULL) { if(sscanf(value,"%lf\n",&fdum) != 1) error("parse LS Offsets dx"); matches->dx=fdum; }
				if(strstr(keyword,"dy") != NULL) { if(sscanf(value,"%lf\n",&fdum) != 1) error("parse LS Offsets dy"); matches->dy=fdum; }
				if(strstr(keyword,"earlyImageJD") != NULL) 
					{ if(sscanf(value,"%lf\n",&fdum) != 1) error("parse LS Offsets earlyImageJD"); matches->jdEarly=fdum; }
				if(strstr(keyword,"lateImageJD") != NULL) 
					{ if(sscanf(value,"%lf\n",&fdum) != 1) error("parse LS Offsets lateImageJD"); matches->jdLate=fdum; }

				if(strstr(keyword,"nx") != NULL) { if(sscanf(value,"%i\n",&idum) != 1) error("parse LS Offsets nx"); matches->nx=idum; }
				...
				if(strstr(keyword,"Mean_sigmaX") != NULL) { 
					if(sscanf(value,"%lf\n",&fdum) != 1) {matches->meanSigmaX=NODATA;}  
					matches->meanSigmaX=fdum; 
				}
				if(strstr(keyword,"Mean_sigmaY") != NULL) {
					if(sscanf(value,"%lf\n",&fdum) != 1) {matches->meanSigmaY=NODATA;}  
					matches->meanSigmaY=fdum; 
				}
			}
		} /* End else */
	} /* End while */
	if(matches->x0 < (NODATA +1)) error("ParseLSOffsetMeta: Invalid x0\n");
	if(matches->y0 < (NODATA +1)) error("ParseLSOffsetMeta: Invalid y0\n");
	if(matches->dx < (NODATA +1)) error("ParseLSOffsetMeta: Invalid dx\n");
	if(matches->dy < (NODATA +1)) error("ParseLSOffsetMeta: Invalid dy\n");
...
void parseKeyValue(char *line,char *keyword,char *value)
{
	...
	*/
	i=0;
	while(line[i] != '='  && i < LINEMAX) {tmp1[i]=line[i];   i++;};
	...
	*/
	j=0; i++;
	while(line[i] != '\0'  && i < LINEMAX) { tmp1[j]=line[i];   i++; j++;};
	if(i == LINEMAX) error("\nparseKeyValue: Invalid keyword pair\n");
	tmp1[i]='\0';
	strcpy(value,tmp1);
}
