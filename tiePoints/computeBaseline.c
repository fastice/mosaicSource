...
void baselineCoeffs(void *x,int i,double *afunc, int ma);
void noRampBaselineCoeffs(void *x,int i,double *afunc, int ma);
void dBpBaselineCoeffs(void *x,int i,double *afunc, int ma);
void dBpQBaselineCoeffs(void *x,int i,double *afunc, int ma);
void bpBaselineCoeffs(void *x,int i,double *afunc, int ma);
void bpbnBaselineCoeffs(void *x,int i,double *afunc, int ma);
void bpdBpBaselineCoeffs(void *x,int i,double *afunc, int ma);
void bnbpdBpBaselineCoeffs(void *x,int i,double *afunc, int ma);
...
{   
	...
	if(tiePoints->quadB == TRUE) nParams=6;
	else if(tiePoints->bpFlag ==TRUE) nParams=1;
	else if(tiePoints->bnbpFlag ==TRUE) nParams=2;
	else if(tiePoints->bnbpdBpFlag ==TRUE) nParams=3;
	...
	*/
	...
	for(k=0; k<=2; k++) {
		...
		if(k == 0) {     
			...
			dBn= tiePoints->dBnorig; dBp =tiePoints->dBporig;
			dBnQ=tiePoints->dBnQorig;dBpQ=tiePoints->dBpQorig;
		} 
		for(i=0; i < nData; i++) {
		...
		if(i1 < 4) error("tiepoints: Insufficient Number (%i)  of Valid tie points \n",i1);
		...
		if(tiePoints->dBpFlag == TRUE) {
			...
			else svdfit((void *)x,y,sig,npts,a,ma,u,v,w, &chisq, &baselineCoeffs);
		}
		svdvar(v,ma,w,Cp);
		if( tiePoints->dBpFlag == FALSE) a[3] =  twok * a[3] / ( inputImage.azimuthSize * inputImage.nAzimuthLooks);
		if(tiePoints->quadB == TRUE) {
			Bn=a[1],Bp=a[2]; dBn=a[4]; dBp=a[3]; dBnQ=a[6]; dBpQ=a[5]; 
			pIndex[1]=1; pIndex[2]=2; pIndex[3]=4; pIndex[4]=3; pIndex[5]=6; pIndex[6]=5;
		} else if(tiePoints->bpFlag==TRUE) {
			Bn=tiePoints->BnCorig; Bp=a[1]; dBn=tiePoints->dBnorig; dBp=tiePoints->dBporig; dBnQ=tiePoints->dBnQorig; dBpQ=tiePoints->dBpQorig;
			pIndex[1]=0; pIndex[2]=1; pIndex[3]=0; pIndex[4]=0; pIndex[5]=0; pIndex[6]=0;
		} else if(tiePoints->bnbpFlag==TRUE) {
			Bn=a[1]; Bp=a[2]; dBn=tiePoints->dBnorig; dBp=tiePoints->dBporig;  dBnQ=tiePoints->dBnQorig; dBpQ=tiePoints->dBpQorig;
			pIndex[1]=1; pIndex[2]=2; pIndex[3]=0; pIndex[4]=0; pIndex[5]=0; pIndex[6]=0;
		}else if(tiePoints->bpdBpFlag==TRUE) {
			Bn=tiePoints->BnCorig; Bp=a[1]; dBn=tiePoints->dBnorig; dBp=a[2]; dBnQ=tiePoints->dBnQorig; dBpQ=tiePoints->dBpQorig;
			pIndex[1]=0; pIndex[2]=1; pIndex[3]=0; pIndex[4]=2; pIndex[5]=0; pIndex[6]=0;
		} else if(tiePoints->bnbpdBpFlag==TRUE) {
			Bn=a[1]; Bp=a[2]; dBn=tiePoints->dBnorig; dBp=a[3];  dBnQ=tiePoints->dBnQorig; dBpQ=tiePoints->dBpQorig;
			pIndex[1]=1; pIndex[2]=2; pIndex[3]=0; pIndex[4]=3; pIndex[5]=0; pIndex[6]=0;
		}  else {
			Bn=a[1]; Bp=a[2]; dBn=a[4]; dBp=a[3];
             		pIndex[1]=1; pIndex[2]=2; pIndex[3]=4; pIndex[4]=3; pIndex[5]=0; pIndex[6]=0;
		}
		varP=varP/npts;
		meanP=meanP/npts;
	...
	for(l1=1; l1 <=6; l1++) {
		...
		for(l2=1; l2 <=6; l2++) {
			Cij=0;
			if(pIndex[l1] > 0 && pIndex[l1] <= ma && pIndex[l2] > 0 && pIndex[l2] <= ma) Cij=Cp[ pIndex[l1] ][ pIndex[l2] ];
