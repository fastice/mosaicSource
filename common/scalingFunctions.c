...
void endScale(outputImageStructure *outputImage,float **vXimage,float **vYimage,float **vZimage,float **errorX,float **errorY, float **scaleX,float **scaleY,float **scaleZ,int statsFlag) {
	...
	for (j=0; j < outputImage->ySize; j++) {
		for(k=0; k < outputImage->xSize; k++) {
			...
			if(scaleZ[j][k] > 0.0)  vZimage[j][k] /= scaleZ[j][k];
			else vZimage[j][k] = -LARGEINT;
			if(statsFlag==FALSE) {
				if(scaleX[j][k] > 0.0)  errorX[j][k] /= (scaleX[j][k]*scaleX[j][k]);
				else errorX[j][k] = -LARGEINT;
				if(scaleY[j][k] > 0.0)  errorY[j][k] /= (scaleY[j][k]*scaleY[j][k]);
				...
				} else {	errorX[j][k] =0.0; errorY[j][k]=0.0;}
