...
int32_t rangeAzimuthToLL(double *rg, double range, double iFloat, double rhoSp, double ReH, double Re,
						 double *lat, double *lon, double *hWGS, inputImageStructure *inputImage, xyDEM *xyDem, double tol, double step)
{
	double R0, ReWGS, rhoWGS, drgStep, R;
	int32_t recycle, nIter;
	recycle = FALSE;
	nIter = 0;
	while (1)
	{
