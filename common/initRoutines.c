...
*/
void setupBuffers(outputImageStructure *outputImage, float ***vXimage, float ***vYimage, float ***vZimage, float ***scaleX,
				  float ***scaleY, float ***scaleZ, float ***vxTmp, float ***vyTmp, float ***vzTmp, float ***sxTmp, float ***syTmp,
				  float ***fScale, float ***errorX, float ***errorY)
{
...
*/
void interpTideError(double *phaseError, inputImageStructure *phaseImage, vhParams *params, double x, double y,
					 double psi, double twok)
{
	double tdTemp, tideError;
	if (phaseImage->tideDiffFlag == TRUE)
	{
		tdTemp = interpTideDiff(x, y, phaseImage->tideDiff);
		if (tdTemp > MINELEVATION)
			phaseImage->tideCorrection = tdTemp * 365.25 / (double)params->nDays;
		else
			phaseImage->tideCorrection = 0.0;
											 ...
											 double *thetaC, double *ReHfixed, double *thetaCfixedReH)
{
	...
	*Re = cP->Re;
	if (cP->ReH != NULL)
		*ReH = cP->ReH[(int32_t)(currentImage->azimuthSize) / 2];
	else
	{
		error(" setup setupGeoConversion *ReH = cP->Re + cP->H");
	}
...
*/
float shelfMaskCorrection(inputImageStructure *currentImage, vhParams *currentParams, unsigned char sMask, double x, double y,
						  double psi, double *sigmaR)
{
	...
	if (sMask == SHELF)
	{
		...
		if (currentImage->tideDiffFlag == TRUE)
		{
			...
			if (tdTemp > MINELEVATION)
				currentImage->tideCorrection = tdTemp * 365.25 / (double)currentParams->nDays;
			else
				currentImage->tideCorrection = 0.0;
		}
				  ...
				  double thetaC, double *ReH, double *Range, double *theta, double *thetaD, double *psi, double zSp)
{
	...
	if (azimuth < cP->azSize && azimuth >= 0)
		*ReH = cP->ReH[(int32_t)azimuth];
	else
		*ReH = cP->ReH[(int32_t)cP->azSize / 2]; /* this should not actually get used, since no valid azimuth */
...
double computeSig2Base(double sinThetaD, double cosThetaD, double azimuth, inputImageStructure *inputImage, Offsets *offsets)
{
	...
	*/
	v[1] = -sinThetaD;
	v[2] = -sinThetaD * normAzimuth;
	v[3] = -cosThetaD * normAzimuth;
	v[4] = -sinThetaD * xsq;
	v[5] = -cosThetaD * xsq;
	if (offsets->deltaB == DELTABNONE)
		v[6] = 1; /* const */
	else
		v[6] = -cosThetaD; /* with state vectors correction to Bp */
	/*
	  C*v
	*/
	for (i = 1; i <= 6; i++)
			  ...
			  inputImageStructure *dPhaseImage, double A[2][2])
{
	...
	if (fabs(alpha) < 0.8 || fabs(alpha) > (2 * PI - 0.8))
		A[0][0] = -LARGEINT;
}
 ...
 */
static int32_t badZ(double z1)
{
	if (z1 <= MINELEVATION || z1 > 10000.0)
		return TRUE;
	return (FALSE);
}
...
*/
void computeVxy(double aP, double dP, double aPe, double dPe, double A[2][2], double B[2][2], double *vx, double *vy,
...
*/
static int32_t inRect(double xP, double yP, double *xR, double *yR)
{
	int32_t i;
	/* Any point not left indicates point outside rectangle for CCW check */
	for (i = 0; i < 4; i++)
		if (isLeft(xP, yP, xR[i], yR[i], xR[i + 1], yR[i + 1]) < 0)
			return (FALSE);
	return TRUE;
}
				  ...
				  int32_t *iMin, int32_t *iMax, int32_t *jMin, int32_t *jMax, outputImageStructure *outputImage)
{
	...
	*iMax = (int32_t)((maxY * KMTOM - outputImage->originY + pad) / outputImage->deltaY);
	*jMax = (int32_t)((maxX * KMTOM - outputImage->originX + pad) / outputImage->deltaX);
	*iMin = max(*iMin, 0);
	*jMin = max(*jMin, 0);
	...
	*/
	for (i = 0; i < 2; i++)
	{
		...
		*/
		...
		{
			minXa = min(minXa, min(xa1, xa2));
			minYa = min(minYa, min(ya1, ya2));
			maxXa = max(maxXa, max(xa1, xa2));
			maxYa = max(maxYa, max(ya1, ya2));
		}
		...
		for (j = 0; j < 2; j++)
		{
			...
			*/
			...
			{
				minXd = min(minXd, min(xd1, xd2));
				minYd = min(minYd, min(yd1, yd2));
				maxXd = max(maxXd, max(xd1, xd2));
				maxYd = max(maxYd, max(yd1, yd2));
			}
			/*
			  Save intersection
			*/
...
void computeSceneAlpha(outputImageStructure *outputImage, inputImageStructure *aOffImage, inputImageStructure *dOffImage,
					   conversionDataStructure *aCp, conversionDataStructure *dCp, xyDEM *dem, int32_t *iMin, int32_t *iMax,
...
/*
  Malloc a floating point32_t image
 */
...
double getReH(conversionDataStructure *cP, inputImageStructure *inputImage, double azimuth)
{
	...
	int32_t index;
	/* for  index inbounds */
	index = (int32_t)min(max(0, azimuth), cP->azSize - 1);
	if (cP->ReH != NULL)
		ReH = cP->ReH[index];
	else
	{
		ReH = cP->Re + inputImage->par.H;
		fprintf(stderr, "*** Warning using fixed ReH ***");
	} /* Should be obsolete*/
...
void initOutputImage(outputImageStructure *outputImage, inputImageStructure inputImage)
{
	...
	*/
	...
	if (outputImage->originX > (LARGEINT - 1))
	...
	*/
	if (inputImage.imageType == COMPLEX)
		outputImage->imageType = COMPLEX;
	else
		outputImage->imageType = FLOAT;
	/*
