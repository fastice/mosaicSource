...
static void unitTCN(double R[3],double V[3],double T[3],double C[3],double N[3]) {
	...
	N[0]=-R[0]/normPos;  N[1]=-R[1]/normPos;   N[2]=-R[2]/normPos;
	/* C = NxV/|NxV| */
	cross(N[0],N[1],N[2],  V[0],V[1],V[2],  &(NxV[0]),&(NxV[1]),&(NxV[2])) ;
	normNxV=norm(NxV[0],NxV[1],NxV[2]);	
	C[0]=NxV[0]/normNxV;  C[1]=NxV[1]/normNxV;   C[2]=NxV[2]/normNxV;
	/* T = CxN*/
...
void svInterpBnBp(inputImageStructure *inputImage, Offsets *offsets, double azimuth,double *bnS,double *bpS ) {
	double azTime;
	int iAzimuth;
	if(offsets->bnS == NULL || offsets->bpS == NULL) svInitBnBp(inputImage,offsets);
...
static  void order1Fit(void *x,int i,double *afunc, int ma) 
{
	svdData *xx;
	double x1,y1;
	xx = (svdData *)x;
...
double svAzOffset(inputImageStructure *inputImage, Offsets *offsets,double range, double azimuth ) {
	 ...
	 */
	if(offsets->azInit != TRUE) svInitAzParams(inputImage,offsets);
	/*
	...
	*/
	...
	if(AZORD == 5) dA+=offsets->azFit[4]*rPoly*rPoly;
	return dA;
}
...
void svInitAzParams(inputImageStructure *inputImage, Offsets *offsets ) {
	...
	double **u,**v,*w,chisq, uSpace[(NAPTS+1)*(AZORD+1)],wSpace[AZORD+1],vSpace[(AZORD+1)*(AZORD+1)], u1[NAPTS+1],v1[(AZORD+1)];
	...
	nPts = n-1;
	if(nPts < 2*NAPTS) error("svInitAzParams : insufficient points to fit");
...
void svTest(inputImageStructure *inputImage, Offsets *offsets ) {
	...
	for(i=0; i < inputImage->azimuthSize; i++) {
		...
		if( i % 1000 == 0) {fprintf(stderr,"%lf %lf  %f %f\n",bTCN1[1],bTCN1[2],bTCN2[1],bTCN2[2]);}
...
void svOffsets(inputImageStructure *image1,inputImageStructure *image2, Offsets *offsets, double *cnstR,double *cnstA) {
	...
	Re = image1->cpAll.Re;	H = image1->par.H;
	if(image1->cpAll.ReH != NULL) ReH =image1->cpAll.ReH[(int) (image1->azimuthSize)/2];
	else ReH = Re + H;
	thetaC = thetaRReZReH(image1->cpAll.RCenter,(Re+0),(Re+H));
	*cnstR=0.0; *cnstA=0.0; nPts=0;
	for(i=0;  i < 5; i++) {
...
void svBaseTCN(double myTime, double dt1t2, stateV *sv1,stateV *sv2,double bTCN[3]) {
	...
	for(i = 0; i < 5; i++) {
		...
		n=(long int)( (myTime2 - sv2->times[1] )/ (sv2->deltaT)+.5);
