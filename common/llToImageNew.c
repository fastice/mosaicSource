...
void initllToImageNew(inputImageStructure *inputImage) {
	...
	inputImage->minLat=1000; inputImage->maxLat=-1000; inputImage->minLon=1000; inputImage->maxLon=-1000;
	for(i=1; i<5; i++) {
	...
	if(inputImage->isInit == -1111) memMode=0;
	else	cp->ReH = NULL;
	cp->prf=par->prf;	
...
static int checkLL(double lat,double lon,inputImageStructure *inputImage) {
	if(lon > 180.) lon-=360.;
	if(lat > (inputImage->maxLat+0.5) || lat < (inputImage->minLat-0.5) ||
	   lon > (inputImage->maxLon+0.5) || lon < (inputImage->minLon-0.5) ) return FALSE;
	return TRUE;
}
...
static void computeSatHeightNew( conversionDataStructure *cp,   inputImageStructure *inputImage,int memMode) {
	...
	if(memMode==1234) {
		...
		*/   
		if(inputImage->passType == DESCENDING) memType=0; /* Default choose by asc/desc */
		else memType=1;
		if(inputImage->memChan==MEM1) memType=0;     /* Overide if mem type gets set */
		else if(inputImage->memChan==MEM2) memType=1;
		if(memType == 0) buf=Dbuf2; else buf=Abuf2;
		cp->rNear = (double *) &(buf[satHpoint]);       satHpoint += sizeof(double)*cp->azSize;
		cp->rFar=   (double *) &(buf[satHpoint]);       satHpoint += sizeof(double)*cp->azSize;
		cp->ReH =   (double *)  &(buf[satHpoint]);      satHpoint += sizeof(double)*cp->azSize;
		if(satHpoint > MAXADBUF) error("satHpoint exceeds buffer size\n");
	} else if(memMode == 999) {
