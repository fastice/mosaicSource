...
void readXYDEMcrop(char *xyFile, xyDEM *xydem, float xmin, float xmax, float ymin, float ymax)
{
	...
	*/
	...
	x1 = (int32_t)((max(xmn, xmin) - xyTemp->x0) / xyTemp->deltaX + 0.5);
	y1 = (int32_t)((max(ymn, ymin) - xyTemp->y0) / xyTemp->deltaY + 0.5);

	x2 = x1 + (int32_t)((min(xmx, xmax) - max(xmn, xmin)) / xyTemp->deltaX + 0.5) + 1;
	y2 = y1 + (int32_t)((min(ymx, ymax) - max(ymn, ymin)) / xyTemp->deltaY + 0.5) + 1;
	xydem->xSize = x2 - x1 + 1;
	xydem->ySize = y2 - y1 + 1;
