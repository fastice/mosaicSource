...
double computeHeading(double lat,double lon,double z, inputImageStructure *inputImage, conversionDataStructure *cP) {
	...
	if(inputImage->lookDir == LEFT) dlat = -0.0025; else dlat = 0.0025;
	Re = cP->Re;
	RNear = cP->RNear;
	z=0.0;
	...
	if(cP->ReH != NULL) {
		...
		ReH = cP->ReH[index];
	} else {    /*ReH = cP->Re + cP->H; */ error("computeHeading - obsolte "); }
	if(range1 < -2000 || range1 > (inputImage->rangeSize+2000)) return(9999.);
	/*
