...
{   
    ...
    if(dem->coordType == LATLON) {
        ...
        t = lonGrid - (double)j;
        u = latGrid - (double)i;
        jSize = dem->lonSize; iSize= dem->latSize;
        ...
        t = xGrid - (double)j;
        u = yGrid - (double)i;
        iSize = dem->lonSize; jSize= dem->latSize;
    } else error("getHeight: Invalid coordinate type\n");
/*
   If location of DEM return border height.
*/
    if(i < 0) return 0; else 
    if(i+2 > iSize) return 0;
    if(j < 0) return 0; else 
    if(j+2 > jSize) return 0;
...
*/
    h = ((1.0 - t)*(1.0 - u) * p1 + t * (1.0 - u) * p2 +
              t * u * p3 +                (1.0 - t) * u* p4);
    if(heightFlag == ELLIPSOIDAL)  return h;
/*
