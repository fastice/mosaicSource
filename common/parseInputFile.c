...
static void parseImageDate(inputImageStructure *inputImage, char *line, char *inputFile) {
	...
	else if(tmp1=strstr(tmp,"MAR"), tmp1 !=  NULL) mm=3;
	else if(tmp1=strstr(tmp,"APR"), tmp1 !=  NULL) mm=4;
	else if(tmp1=strstr(tmp,"MAY"), tmp1 !=  NULL) mm=5;
	else if(tmp1=strstr(tmp,"JUN"), tmp1 !=  NULL) mm=6;
	else if(tmp1=strstr(tmp,"JUL"), tmp1 !=  NULL) mm=7;
	else if(tmp1=strstr(tmp,"AUG"), tmp1 !=  NULL) mm=8;
	else if(tmp1=strstr(tmp,"SEP"), tmp1 !=  NULL) mm=9;
	else if(tmp1=strstr(tmp,"OCT"), tmp1 !=  NULL) mm=10;
	else if(tmp1=strstr(tmp,"NOV"), tmp1 !=  NULL) mm=11;
	else if(tmp1=strstr(tmp,"DEC"), tmp1 !=  NULL) mm=12;
	else error("++invalid date in %s %s\n",inputFile,tmp);
	sscanf((tmp1+3),"%i\n",&yyyy);
...
static void parseTime(inputImageStructure *inputImage, char *line, int eod) {
	if(sscanf(line,"%i %i %lf\n",&(inputImage->par.hr), &(inputImage->par.min),
		  &(inputImage->par.sec) ) != 3) error("Invalid image start time \n %s\n",line);
	if(eod == TRUE) error("missing data after time");
}

static void parsePRF(inputImageStructure *inputImage, char *line,int eod) {
	if(sscanf(line,"%lf\n",&(inputImage->par.prf)) != 1) error("Invalid prf");
	if(eod == TRUE) error("missing data after prf");	
}

static void parseLambda(inputImageStructure *inputImage, char *line,int eod) {
	if(sscanf(line,"%lf\n",&(inputImage->par.lambda)) != 1) 
		error("Invalid wavelength");
	if(eod == TRUE) error("missing data after lamda");
}



static int parseStateVectors(FILE *fp, int lineCount,inputImageStructure *inputImage,char *inputFile) {
	stateV *sv;
	char line[512];
	int  eod, i;
	...
	*/
	...
	if(sscanf(line,"%i",&(sv->nState)) != 1) error("missing number of state vectors");
	if(sv->nState < MNST || sv->nState > MXST) error("\nInvalid number of state vectors - %i\n",sv->nState);
     
	lineCount=getDataString(fp,lineCount,line,&eod); /* Get next line */
	if(eod == TRUE) error("missing state vector info at line %i\n",lineCount);
	...
	*/
	for(i=1; i <= sv->nState; i++) {
		/* Position */
		lineCount=getDataString(fp,lineCount,line,&eod); /* Get next line */
		if( sscanf(line,"%le %le%le", &(sv->x[i]), &(sv->y[i]),&(sv->z[i])) != 3 ) error("Invalid state x,y,z %i %s\n",lineCount,inputFile);  	                   
		/* Velocity */
		lineCount=getDataString(fp,lineCount,line,&eod); /* Get next line */
		if( sscanf(line,"%le %le%le", &(sv->vx[i]), &(sv->vy[i]),&(sv->vz[i]) ) != 3) error("Invalid vx,vy,vz at line %i %s\n",lineCount,inputFile); 
		sv->times[i] = sv->t0 + (i-1)*sv->deltaT;
	}
	return(lineCount);
}

static void parseDeltaT(inputImageStructure *inputImage, long julDayInt, char *line) {
	SARData *par;
	...
	if(strstr(line,"deltaT") != NULL) sscanf((line+6),"%lf\n",&deltaTCorrect);
	par->sec += deltaTCorrect;
	if(par->sec > 60.0) {par->sec -= 60.0; par->min +=1;}
...
static void parseSize(inputImageStructure *inputImage, char *line, char *inputFile) {
	...
	if( sscanf(line,"%i%i%i%i",&nr,&na,&nlr,&nla) != 4) error("%s  %i %s %s", "parseInputFile -- Missing image size parameters","File: ",inputFile); 
...
static void parseAscDesc(inputImageStructure *inputImage, char *line, char *inputFile) {
	int i;
	for(i=0; i < 256 && line[i] != '\0'; i++)  line[i] = (char)tolower(line[i]);
	...
	}  else error("%s %s",inputFile,"parseInputFile: missing ascending or descending flag\n");     
}
...
static void parseLookDir(inputImageStructure *inputImage, char *line, char *inputFile) {
	...
	} else error("%s %s",inputFile,"no left/right in geodata file \n");
}
...
void parseInputFile(char *inputFile, inputImageStructure *inputImage)
{
	...
	if(eod == TRUE || strstr(line,"state") == NULL )  error("no state flag present in geodat");
	/* Get corrected time */
	lineCount=getDataString(fp,lineCount,line,&eod);
	...
	/* end add */
	if(inputImage->lookDir == LEFT) par->lookDir=-1.0; 	else par->lookDir=1.0;        
	/*	  close up and exit        */
	fclose(fp);
	return;
}
