...
static void getTriBox(int32_t index, int32_t *iMin, int32_t *iMax, int32_t *jMin, int32_t *jMax,
					  irregularData *data, outputImageStructure *outputImage);
static int32_t inTriangle(double x, double y, int32_t n, irregularData *data);
static int32_t goodTriangle(int32_t n, irregularData *data);
static void linTriInterp(double x, double y, int32_t n, irregularData *data, double *vx, double *vy);
...
static void linTriInterp(double x, double y, int32_t n, irregularData *data, double *vx, double *vy)
{
	...
	*/
	...
	ax = -f * (ykj * zljx - zkjx * ylj);
	ay = -f * (ykj * zljy - zkjy * ylj);
	bx = -f * (zkjx * xlj - xkj * zljx);
	by = -f * (zkjy * xlj - xkj * zljy);
	cx = -ax * ux[1] - bx * uy[1] + zkx;
	cy = -ay * ux[1] - by * uy[1] + zky;
...
static int32_t goodTriangle(int32_t n, irregularData *data)
{
	...
	x1 = data->x[data->link[n * 3]];
	y1 = data->y[data->link[n * 3]];
	x2 = data->x[data->link[n * 3 + 1]];
	y2 = data->y[data->link[n * 3 + 1]];
	x3 = data->x[data->link[n * 3 + 2]];
	y3 = data->y[data->link[n * 3 + 2]];
	/*
...
static int32_t inTriangle(double x, double y, int32_t n, irregularData *data)
{
	...
	vx1 = data->x[data->link[n * 3]] - x;
	vy1 = data->y[data->link[n * 3]] - y;
	vx2 = data->x[data->link[n * 3 + 1]] - x;
	vy2 = data->y[data->link[n * 3 + 1]] - y;
	vx3 = data->x[data->link[n * 3 + 2]] - x;
	vy3 = data->y[data->link[n * 3 + 2]] - y;
					  ...
					  irregularData *data, outputImageStructure *outputImage)
{
	...
	*iMin = max(*iMin, 0);
	*jMin = max(*jMin, 0);
	*iMax = min(outputImage->ySize - 1, *iMax);
	*jMax = min(outputImage->xSize - 1, *jMax);
}
