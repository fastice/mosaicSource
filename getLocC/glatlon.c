...
void glatlon( SARData *sarD, stateV *sv, int nlr, int nla, double ***lat1,double ***lon1,int ma, int mr, double deltaT)
{
	...
	*/
	...
	for(i=1; i <= sv->nState; i++) {
		if(sv->times[i] > 86400) endofday=TRUE;
	}	
	/*
	  Set up space
	*/
	if(latSpace  ==NULL)  latSpace = dmatrix(0,ma-1,0,mr-1);
	if(lonSpace ==NULL)  lonSpace=dmatrix(0,ma-1,0,mr-1);
	if(naSpace  ==NULL)  naSpace = dmatrix(0,ma-1,0,mr-1);
	if(nrSpace  ==NULL)   nrSpace = dmatrix(0,ma-1,0,mr-1);
	na=naSpace; nr=nrSpace; lat=latSpace; lon=lonSpace;
	*lat1 = lat;
	*lon1 = lon;
	/*
	...
	for(i=0; i < ma; i++) {
		...
		for(j=0; j < mr; j++) {
			...
			if(sv->nState > NUSESTATE) {
				n=(long int)((pTime-sv->times[1])/(sv->times[2]-sv->times[1])+.5);
