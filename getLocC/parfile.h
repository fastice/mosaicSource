...
typedef struct
{
   ...
   int32_t nr_offset;      /* offset in (samples) to first range sample to process,
   ...
   double prfrac;          /* fraction of doppler bandwidth to process
                                    (0.0 -- 1.0) */
   int32_t nprs_az;        /* initial azimuth presumming factor
                              (4 for a 4:1 presum, for example) */
   int32_t loff;           /* offset in echoes from the start of SAR signal
                                 data file to the first presum output record */
   int32_t nl;             /* # echoes to process/simulate */
   int32_t nrfft;          /* # of range samples to process (set equal to the
                                 range FFT size, power of 2) */
   int32_t nsub;           /* # sub-apertures to divide the presummed doppler
                                 spectrum */
   int32_t nlr, nlaz;      /* # range looks, # azimuth looks/sub-aperture */
   ...
   double azimsp, azimres; /* azimuth image pixel spacing, azimuth image
                              resolution (m) */
   int32_t nrs, nazs;      /* image width in range pix, length in azimuth pix */
   int32_t nstate;         /* number of state vectors */
...
void glatlon(PROC_PAR *par, double ***lat1, double ***lon1, int32_t ma, int32_t mr,
             double deltaT);
...
void correctTime(PROC_PAR *par, double *squint, double noffset,
                 double *tskew, double *toffset, int32_t squintTime);
