...
static void mallocTmpBuffers(float ***imageTmp, float ***scaleTmp,  outputImageStructure *outputImage,float ***psiBuf, float ***psiBufTmp,float ***gBuf, float ***gBufTmp)
{
	...
	myBuff+=bufSize;	
	if(bufI == NULL) error("Unable to malloc buffer 1 in mallocTmpBuffers %d %d\n",myBuff,bufSize);		
	bufS=(float *)malloc((size_t)bufSize);
	myBuff+=bufSize;		
	if(bufS == NULL) error("Unable to malloc buffer 2 in mallocTmpBuffers %d %d\n",myBuff,bufSize);		
	bufP=(float *)malloc((size_t)bufSize);
	myBuff+=bufSize;		
	if(bufP == NULL) error("Unable to malloc buffer 3 in mallocTmpBuffers %d %d\n",myBuff,bufSize);		
	bufPTmp=(float *)malloc((size_t)bufSize);
	myBuff+=bufSize;		
	if(bufPTmp == NULL) error("Unable to malloc buffer 4 in mallocTmpBuffers  %d %d\n",myBuff,bufSize);		
	bufG=(float *)malloc((size_t)bufSize);
	myBuff+=bufSize;		
	if(bufG == NULL) error("Unable to malloc buffer 5 in mallocTmpBuffers %d %d\n",myBuff,bufSize);		
	bufGTmp=(float *)malloc((size_t)bufSize);
	myBuff+=bufSize;	
	if(bufGTmp == NULL) error("Unable to malloc buffer 6 in mallocTmpBuffers %d %d\n",myBuff,bufSize);		
