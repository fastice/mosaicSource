...
static void geoMosaicScaling(inputImageStructure *inputImage, float **image, float **imageTmp, float **psiBuf,
			     ...
			     float **scale, float **scaleTmp, void *dem, outputImageStructure *outputImage, int orbitPriority,
			     int imageDate, int iMin, int iMax, int jMin, int jMax);
static void finalReScale(outputImageStructure *outputImage, float **image, float **scale, int orbitPriority);
static void initImageBuffers(outputImageStructure *outputImage, float **scaleTmp, int orbitPriority, float **psiBuf,
			     float **psiBufTmp , float **gBuf, float **gBufTmp );
...
static void AbAg(double x, double y, double azimuth, inputImageStructure *inputImage,
		 outputImageStructure *outputImage, void *dem,double *Ab, double *Ag, double *shadow, int recycle) {
...
static double boxOverlap(double range0, double  azimuth0, double range1, double azimuth1, double dx) {
	...
	/* Compute box corners */
	r01 = range0 - dx;	r02 = range0 + dx;
	a01 = azimuth0 - dx;	a02 = azimuth0 + dx;
	r11 = range1 - dx;	r12 = range1 + dx;
	a11 = azimuth1 - dx;	a12 = azimuth1 + dx;
	/* No overlap cases, so return 0 */
	if(r12 < r01 || r11 > r02) return 0.0;
	if(a12 < a01 || a11 > a02) return 0.0;	 
...
static double areaAboutXY(double range, double azimuth, double x, double y, inputImageStructure *inputImage,
			  outputImageStructure *outputImage, void *dem, double value, double *test, int recycle, double *Ab, double *Ag) 
{
	extern int HemiSphere;
	extern double Rotation;
	...
	for(i=-nx; i <= nx; i++ ) {
		...
		for(j=-ny; j <= ny; j++ ) {
			if(  !( (i == 0) && (j  == 0) ) ) { /* Don't redo (0,0) case */
...
void makeGeoMosaic(inputImageStructure *inputImage, outputImageStructure outputImage,
		   void *dem, int nFiles, int maxR, int maxA, char **imageFiles,float fl, int smoothL,
		   int smoothOut, int orbitPriority, float ***psiData,float ***gamma)
{   
	extern int HemiSphere;
	extern double Rotation;
				...
				*/
				if( (S1Cal & TRUE) == TRUE) {
					if( areaAboutXY(range, azimuth, x, y, &(inputImage[i]), &outputImage, dem,
							value, &test, recycle, &Ab, &Ag) < -0.001) shadow=TRUE;
					AbCum=Ab; AgCum=Ag;
					recycle=TRUE;
				}
				if(value > 0 ) {
...
static void geoMosaicScaling(inputImageStructure *inputImage, float **image, float **imageTmp,float **psiBuf, float **psiBufTmp,
			     float **gBuf, float **gBufTmp, float **scale, float **scaleTmp, void *dem, outputImageStructure *outputImage, int orbitPriority,
			     int imageDate, int iMin, int iMax, int jMin, int jMax) {
	extern int HemiSphere;
	extern double Rotation;	
	extern int hybridZ;
...
static void mallocTmpBuffers(float ***imageTmp, float ***scaleTmp, outputImageStructure *outputImage,float ***psiBuf, float ***psiBufTmp, float ***gBuf, float ***gBufTmp)
{
	...
	myBuff+=bufSize;	
	if(bufI == NULL) error("Unable to malloc buffer 1 in mallocTmpBuffers already allocated %f new buf %f\n\n", myBuff/1e6, bufSize/1e6);		
	bufS=(float *)malloc((size_t)bufSize);
	myBuff+=bufSize;		
	if(bufS == NULL) error("Unable to malloc buffer 2 in mallocTmpBuffers already allocated %f new buf %f\n\n", myBuff/1e6, bufSize/1e6);		
	bufP=(float *)malloc((size_t)bufSize);
	myBuff+=bufSize;		
	if(bufP == NULL) error("Unable to malloc buffer 3 in mallocTmpBuffers already allocated %f new buf %f\n\n", myBuff/1e6, bufSize/1e6);		
	bufPTmp=(float *)malloc((size_t)bufSize);
	myBuff+=bufSize;		
	if(bufPTmp == NULL) error("Unable to malloc buffer 4 in mallocTmpBuffers  already allocated %f new buf %f\n\n", myBuff/1e6, bufSize/1e6);		
	bufG=(float *)malloc((size_t)bufSize);
	myBuff+=bufSize;		
	if(bufG == NULL) error("Unable to malloc buffer 5 in mallocTmpBuffers already allocated %f new buf %f\n\n", myBuff/1e6, bufSize/1e6);		
	bufGTmp=(float *)malloc((size_t)bufSize);
	myBuff+=bufSize;	
	if(bufGTmp == NULL) error("Unable to malloc buffer 6 in mallocTmpBuffers already allocated %f new buf %f\n\n", myBuff/1e6, bufSize/1e6);		
